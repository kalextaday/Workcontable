...
/* Created on:     04/01/2020 21:38:42                          */
/*==============================================================*/
...
create table LIBRO_CONTABLE (
   ID_LIB_CONTABLE      NUMBER(8)             not null,
...
create table PRESUPUESTO (
   ID_PRESUPUESTO       NUMBER(8)             not null,
   ID_USUARIO           NUMBER(8)             not null,
   FECHA_INICIO         TIMESTAMP,
   FECHA_FIN            TIMESTAMP,
...
create table REG_DET_LIBROS (
   ID_REG_DET_LIB       NUMBER(10)            not null,
   ID_TRANS_REAL        NUMBER(8)             not null,
   ID_LIB_CONTABLE      NUMBER(8)             not null,
...
create table REG_DET_PRESUPUESTOS (
   ID_REG_DET_PRE       NUMBER(18)            not null,
   ID_PRESUPUESTO       NUMBER(8)             not null,
...
create table TRANSACCION_PRE (
   ID_TRANS_PRE         NUMBER(8)             not null,
   RUB_PRE_ID           NUMBER(8)             not null,
...
alter table LIBRO_CONTABLE
   add constraint FK_LIBRO_CO_RELATIONS_USUARIO foreign key (ID_USUARIO)
      references USUARIO (ID_USUARIO);

alter table PRESUPUESTO
   add constraint FK_PRESUPUE_RELATIONS_USUARIO foreign key (ID_USUARIO)
      references USUARIO (ID_USUARIO);

alter table REG_DET_LIBROS
   add constraint FK_REG_DET__RELATIONS_TRANSACC foreign key (ID_TRANS_REAL)
      references TRANSACCION_REAL (ID_TRANS_REAL);

alter table REG_DET_LIBROS
   add constraint FK_REG_DET__RELATIONS_LIBRO_CO foreign key (ID_LIB_CONTABLE)
      references LIBRO_CONTABLE (ID_LIB_CONTABLE);

alter table REG_DET_PRESUPUESTOS
   add constraint FK_REG_DET__RELATIONS_PRESUPUE foreign key (ID_PRESUPUESTO)
      references PRESUPUESTO (ID_PRESUPUESTO);

alter table TRANSACCION_PRE
   add constraint FK_TRANSACC_PRE_RUBRO foreign key (RUB_PRE_ID)
...
alter table TRANSACCION_REAL
   add constraint FK_TRANSACC_REAL_RUBRO foreign key (ID_RUBRO)
      references RUBRO (ID_RUBRO);
